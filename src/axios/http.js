...
export const get = ({ url, data, headers }) =>
  response(service.get(url, { params: data, headers }));
export const post = ({ url, data, headers }) =>
  response(service.post(url, data, { headers }));
export const put = ({ url, data, headers }) =>
  response(service.put(url, data, { headers }));
export const del = ({ url, data, headers }) =>
  response(service.delete(url, { data, headers }));

const response = axiosObj =>
  axiosObj.then(res => res.data).catch(err => errHandle(err));

const errHandle = err => {
  // 判断上下文是“接口状态码”还是“HTTP状态码”
  let code = err.data ? err.data.code : err.response.status;
  // 是否需要重定向到指定页
  RedirectMap.hasOwnProperty(code) &&
    window.location.replace(RedirectMap[code]);
  err.message = ServerCodeMap[code] || '接口异常';
  // 若code是400，则不会弹出message
  code !== ServerCode.CONTINUE ? console.log(err.message) : console.error(err);
